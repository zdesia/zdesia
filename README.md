##### Hello and welcome!

### Here you can find some of my projects in **Data Analysis** and **Machine Learning**


-  **Programming languages :**  `Python`  `SQL`   
-  **Tools :** `Jupyter`  `Git`  `DataLens`   
-  **Libraries :** `Pandas` `NumPy` `Matplotlib` `Seaborn` `Scikit-learn` `SkiPy` `PyTorch` `Keras` `BeautifulSoup` `Request` `PySpark`




---

<p align="center"> <b>    </b> </p align="center">

 ### Projects
| **Project** | **Task** | **Status** |
| -------------------- | :--------------------- |:---------------------------:|
| [Predictive model for Lenta LLC]() | Проектное участие в кросс-функциональном хакатоне по разработке ML-продукта предсказательной модели для ООО “Лента”. |Completed|
| [Forecasting of electricity consumption in the region!image]()| GlowByte hackaton | Completed |
| [Machine learning to predict a used car price]() | Kaggle competition | Complited |
|Churn|Churn|Complited|  
|Определение возраста покупателей по фотографии|Построить нейронную сеть, которая по фотографии определит приблизительный возраст человека|Complited|

---

#### My education
- 2012 	- Bachelor’s Degree in Engineering, Chemical Technology and Biotechnology, St. Petersburg State University of Cinema and Television
- 2015	- Master of Science in Chemical Technology, St. Petersburg State University of Cinema and Television
- 2023	- Data Scientist professional training course, Yandex Practicum
- 2023	- Deep Learning course, Deep Learning School FPMI of MIPT

**Additional courses:**
- Base statistics, Anatoliy Karpov on Stepik
- Git basics, Yandex.Practicum
- SQL, Gleb Mikhailov on Stepik

---
#### Contacts:
<p align="left">
<a href="https://linkedin.com/in/olesiakrugliakova/" target="blank"><img align="center" src="https://img.icons8.com/color/96/linkedin.png" alt="linkedin.com/in/olesiakrugliakova/" height="30" width="30" /></a>
<a href="https://t.me/inogdazdes" target="blank"><img align="center" src="https://img.icons8.com/fluency/96/telegram-app.png" alt="inogdazdes" height="30" width="30" /></a>
<a href="mailto:zdes.krugliakova@gmail.com" target="blank"><img align="center" src="https://img.icons8.com/color/96/gmail-new.png" alt="an.zdes.krugliakova@gmail.com" height="30" width="30" /></a>
